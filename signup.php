...
<meta name="keywords" content="R web design" />
...
$conn = mysqli_connect("localhost","root","","rentanything")
	...
	$email = $_POST['email'];
 	...
 	$pass =$_POST['password'];
 		...
 		$sql= "INSERT INTO tbluser(userName,userEmail,userPhone,userNic,userAddress,userPassword) VALUES ('{$name}','{$email}','{$phone}','{$cnic}','{$address}','{$pass}')"; 
...
?>
				...
				<form method="post">
					<input type="text" name="name" placeholder="Your Name"  maxlength="25" required="required" pattern="^[A-Za-z -]+$" title="Only Letters Are Allowed" required=""> 

					<input type="email" name="email"  placeholder="Your Email"  placeholder="Your Email" required="" title="Invalid email address" required=""> 

					<input type="tel" name="phone"  placeholder="0900-1234567" data-inputmask="'mask': '0399-99999999'"  required="" title="Only Allow This Pattern 0900-1234567"> 

					 <input type="text"  name="cnic"  data-inputmask="'mask': '99999-9999999-9'"  placeholder="XXXXX-XXXXXXX-X" required="" title="Only Allow This Pattern XXXXX-XXXXXXX-X">

					  <input type="text" name="address"  placeholder="Residential Address" required="">
					 
					<input type="password" name="password"  pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" placeholder="Password" required="" title="Minimum Length 8 (1 upper letter 1 lower letter 1 special character must)">
				...
				<!--<h6> <a href="signup.php">Sign In Now</a> </h6>-->			</div>
		</div>
	</section>
	<!-- //sign up form -->
		...
		<footer>
			...
			<div class="container">
				...
				<div class="copyrights">
					<p> © 2021 RentAnything. All Rights Reserved | Design by RentAnthing  <a href="index.php"> -</a></p>
				</div>
				<div class="clearfix"></div>
			</div>
		</div>
